...
The preparation candidate is `1.0.0-rc.3`. It is a GitHub **draft**, available
...
RC3 replaces RC2's mismatched Flatpak locale inventory and GitHub-normalized
Debian download name. RC1/RC2 tags and original artifacts remain unchanged;
use only the freshly verified RC3 downloads below, not renamed older packages.
...
```bash
set -euo pipefail
gh release download v1.0.0-rc.3 --repo brdweb/omacalendar \
  --pattern 'omacalendar-1.0.0rc3-1-x86_64.pkg.tar.zst' \
  --pattern 'omacalendar_1.0.0-rc.3-1_amd64.deb' \
  --pattern 'omacalendar-1.0.0-rc.3-linux-x86_64.flatpak' \
  --pattern SHA256SUMS --pattern '*documentation.tar.gz'
```

The Debian asset uses a GitHub-safe hyphenated filename. No local rename is
needed. Its internal Debian version retains `~` for correct prerelease ordering;
do not modify the downloaded package or its checksum manifest.

For each selected install package, set `package` to its exact filename and verify:

```bash
set -euo pipefail
package=omacalendar-1.0.0rc3-1-x86_64.pkg.tar.zst
...
gh attestation verify "./$package" --repo brdweb/omacalendar \
  --source-ref refs/tags/v1.0.0-rc.3 \
...
```bash
set -euo pipefail
documentation=omacalendar-1.0.0-rc.3-documentation.tar.gz
...
```bash
sudo pacman -U ./omacalendar-1.0.0rc3-1-x86_64.pkg.tar.zst
...
Arch prerelease `1.0.0rc3` and Debian `1.0.0~rc.3-1` sort below stable `1.0.0`.
