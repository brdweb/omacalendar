...
docker run --rm \
  ...
  bash packaging/release/build-deb-release.sh 1.0.0-rc.5 /output
```
...
field: `1.0.0-rc.5` becomes `1.0.0~rc.5-1`. The download filename is separately
`omacalendar_1.0.0-rc.5-1_amd64.deb`, using GitHub-safe characters because
...
docker run --rm \
  ...
  bash /source/packaging/release/test-deb-package.sh \
    1.0.0-rc.5 /packages/omacalendar_1.0.0-rc.5-1_amd64.deb
```
...
Follow the [candidate download and verification guide](https://github.com/brdweb/omacalendar/blob/v1.0.0-rc.5/docs/INSTALL.md)
...
```bash
set -euo pipefail
grep ' omacalendar_1.0.0-rc.5-1_amd64.deb$' SHA256SUMS | sha256sum --check
...
[the backup and recovery guide](https://github.com/brdweb/omacalendar/blob/v1.0.0-rc.5/docs/BACKUP_AND_RECOVERY.md).
...
credentials. See [the uninstall guide](https://github.com/brdweb/omacalendar/blob/v1.0.0-rc.5/docs/UNINSTALL.md) for explicit
